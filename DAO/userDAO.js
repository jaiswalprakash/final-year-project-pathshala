...
const UserDAO = {
  ...
  login: async function (bodyData) {
    ...
    if (!data) {
      ...
      return {
        message: constant.MESSAGE.LOGIN.EMAIL_NOT_VALID,
        status: constant.HTML_STATUS_CODE.INVALID_DATA,
        code: code,
      };
    }
    ...
    if (!isMatch) {
      ...
      return {
        message: constant.MESSAGE.LOGIN.WRONG_PASSWORD,
        status: constant.HTML_STATUS_CODE.INVALID_DATA,
        code: code,
      };
    } else {
      if (data && data.role == "STUDENT") {
        ...
        return {
          message: constant.MESSAGE.LOGIN.SUCCESS,
  ...
  forgot: async function (bodyData) {
    ...
    if (data.length > 0) {
      ...
      sgMail
        ...
        .then(() => {
          return {
            message: constant.MESSAGE.LOGIN.EMAIL_SUCCESS,
            status: constant.HTML_STATUS_CODE.SUCCESS,
          };
        })
        .catch((error) => {
          return {
            message: constant.MESSAGE.COMMON.INTERNAL_ERROR,
            status: constant.HTML_STATUS_CODE.UNAUTHORIZED,
          };
        });
    } else {
      return {
  ...
  updatePassword: async function (bodyData) {
    ...
    try {
      if (bodyData.otp === JSON.stringify(myCache.get("otp"))) {
        ...
        if (data.nModified === 1) {
          return {
            message: constant.MESSAGE.LOGIN.PASSWORD_CHANGED,
            status: constant.HTML_STATUS_CODE.SUCCESS,
          };
        } else {
          return {
            message: constant.MESSAGE.LOGIN.PASSWORD_CHG_FAILED,
            status: constant.HTML_STATUS_CODE.BAD_REQUEST,
          };
        }
      } else {
        return {
      ...
      res
        ...
        .send(constant.MESSAGE.COMMON.INTERNAL_ERROR);
    }
  },
  wishList: async (bodyData) => {
    data = await UserModel.updateOne(
      { _id: bodyData.userId },
      { $addToSet: { wishList: bodyData.topicId } }
    );
    if (data) {
      return true;
    }
